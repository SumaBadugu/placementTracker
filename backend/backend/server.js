...
app.post("/api/addCompany", (req, res) => {
  ...
  const query = "INSERT INTO companies (cname, trounds) VALUES (?, ?)";

  db.query(query, [cname, trounds], (err, result) => {
    if (err) {
...
app.delete("/api/deleteCompany/:cname", (req, res) => {
  const { cname } = req.params;

  const query = "DELETE FROM companies WHERE cname = ?";
