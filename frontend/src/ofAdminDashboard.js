...
const OfAdminDashboard = () => {
    useEffect(() => {
        fetch("http://localhost:5000/api/ofcompanies")
            .then(res => res.json())
    ...
    fetch(`http://localhost:5000/api/ofbranchPassedCount?branch=${branch}&company=${companyName}`)
        .then(res => res.json())
        .then(data => {
    ...
    const handleShowDetails = (company) => {
        fetch(`http://localhost:5000/api/ofpassedStudents/${company.ocname}`)
            .then(res => res.json())
            .then(data => {
    ...
    const handleAddCompany = () => {
        if (newCompany.name && newCompany.rounds) {
            // API call to backend
            fetch("http://localhost:5000/api/ofaddCompany", {
                method: "POST",
                headers: {
    ...
    const handleRemoveCompany = () => {
        ...
        fetch(`http://localhost:5000/api/ofdeleteCompany/${selectedCompany}`, {
            method: "DELETE",
        })
            .then(res => res.json())
            .then(data => {
